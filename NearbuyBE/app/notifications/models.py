...
class ItemCategory(Base):
    ...
    item_id = Column(
        PGUUID(as_uuid=True),
        ForeignKey("lists_items.item_id", ondelete="CASCADE"),
        primary_key=True,
        index=True,
    )
    category_id = Column(
...
class DeviceToken(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "expo_push_token", name="uniq_user_expo_token"),
    )
...
class UserStoreProximity(Base):
    ...
    store_id = Column(
        Integer, ForeignKey("stores.store_id", ondelete="CASCADE"), nullable=False
    )
