...
def test_predict_categories_shape():
    response = client.get("/predict_categories", params={"product_name": "Milk", "top_k": 3})
...
def test_recommend_similar_products(top_k):
    response = client.get(
        ...
        params={"product_name": "iphone", "top_k": top_k}
    )
    assert response.status_code == 200, response.text
    data = response.json()
...
def test_recommend_by_list_name():
    ...
    response = client.get(
        "/recommend_by_list_name",
        params={"list_name": "Groceries"}
    )
    assert response.status_code == 200, response.text
    data = response.json()
