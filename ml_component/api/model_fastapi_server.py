...
from ml_component.locations_based_recommender_agent.product_availability_agent import (
    ProductAvailabilityAgent,
)
...
def set_list_recommender(k: int = 2, top_k_per_product: int = 1, final_m: int = 10):
    ...
    list_recommender = ListBasedRecommender(
        k=k, top_k_per_product=top_k_per_product, final_m=final_m
    )
...
def predict_categories(product_name: str, top_k: int = 2):
    if product_recommender is None:
        raise HTTPException(
            status_code=400, detail="ProductRecommender not initialized"
        )
    return {
        "product_name": product_name,
        "predicted_categories": product_recommender.predict_top_categories(
            product_name, top_k
        ),
    }
...
def recommend_similar_products(product_name: str, top_k: int = 5):
    ...
    return {
        "product_name": product_name,
        "similar_products": product_recommender.find_similar_products(
            product_name, top_k
        ),
    }
...
def recommend_by_list_name(list_name: str):
    if list_recommender is None:
        raise HTTPException(
            status_code=400, detail="ListBasedRecommender not initialized"
        )
    return {
        "list_name": list_name,
...
def check_product_availability(product: str, store: str):
    ...
    try:
        ...
        return {"product": product, "store": store, **result}
