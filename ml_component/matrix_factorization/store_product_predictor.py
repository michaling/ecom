...
product_id_mapping = {
    product_id: idx for idx, product_id in enumerate(df["product_id"].unique())
}
store_id_mapping = {
    store_id: idx for idx, store_id in enumerate(df["store_id"].unique())
}
...
reader = Reader(
    rating_scale=(0, 1)
)  # Rating scale between 0 and 1 (for product availability)
...
random_store_id_int = random.choice(
    df["store_id"].unique()
)  # Random store ID from the dataset
random_product_id_int = random.choice(
    df["product_id"].unique()
)  # Random product ID from the dataset
...
random_store_id_uuid = list(store_id_mapping.keys())[
    list(store_id_mapping.values()).index(random_store_id_int)
]
random_product_id_uuid = list(product_id_mapping.keys())[
    list(product_id_mapping.values()).index(random_product_id_int)
]
